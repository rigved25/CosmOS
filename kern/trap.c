...
trap_dispatch(struct Trapframe *tf)
{
	...
	switch (tf->tf_trapno)
	{
		...
		// default:
		// {
		// 	cprintf("Unhandled Exception/ Interrupt occured.");
		// 	return;
		// }
	}
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// //page not avail for user or not writable, destroy
	// if(tf->tf_err & FEC_PR) {
	// 	struct PageInfo *new_pp = page_alloc(ALLOC_ZERO);
	// 	if (!new_pp) {
	// 		panic("page_fault_handler :: Page allocation failed");
	// 	}

	// 	if (page_insert(curenv->env_pgdir, new_pp, (void *)fault_va, PTE_U | PTE_W) < 0) {
	// 		// Unable to insert the new page into the address space
	// 		page_free(new_pp); // Free the newly allocated page
